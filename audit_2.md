...
| 7 | Add ESLint + typecheck scripts | ✅ DONE | No lint/typecheck scripts — no automated code quality enforcement | `npm run lint` (ng lint with @angular-eslint v20), `npm run typecheck` (tsc --noEmit for app + spec), `npm run format`/`format:check` (prettier). ESLint config: stylistic TS rules, Angular plugin rules (directive/component selectors, lifecycle, contextual), `no-explicit-any` warn, `no-unused-vars` warn with `^_` escape, template a11y rules (alt-text, label-has-associated-control, click-events-have-key-events). Spec files relaxed: no-empty-function, no-explicit-any, no-unused-vars off. Fixed 1 pre-existing lint warning: removed unused `PendingChange` interface from `level-permission-matrix.component.ts`. Build clean, 178/178 tests pass |
...
| 10 | Replace hardcoded hex colors with CSS variables | ✅ DONE | Hardcoded hex values in 4 SCSS files: `#f5f5f5`, `#d32f2f`, `#666`, `#e0e0e0`, `#fafafa`, `#22c55e`, `#16a34a` — broke theming, blocked dark mode | Added 5 CSS variables to `styles.scss`: `--color-success`, `--color-success-dark`, `--color-text-secondary`, `--color-border`, `--color-bg-subtle`. Replaced all hardcoded hex in `login.component.scss`, `user-permission-override.component.scss`, `forbidden.component.scss`, `_list-page.scss`. Used `color-mix()` for semi-transparent error background. Build clean, 175/175 tests pass |
| 11 | Replace manual `Subscription` with `takeUntilDestroyed` | ✅ DONE | List components used `Subject` + `Subscription` + `debounceTime` + `ngOnDestroy` for search debouncing | Already resolved by #13 (Extract DataTableState composable). `DataTableState` uses `inject(DestroyRef)` + `takeUntilDestroyed`. No manual `Subscription` or `ngOnDestroy` remains anywhere in codebase |
...
| 14 | Add typed forms | ✅ DONE | All 3 form components used untyped `FormGroup` — `form.get('x')` returned `AbstractControl | null`, `form.value` was `Partial<any>`, required `as` casts on `getRawValue()` | Converted to `FormGroup<T>` with `FormControl<T>` interfaces. Used `fb.nonNullable.control()` for all controls. Removed `as` casts. Getter methods return typed `FormControl<T>`. `patchValue` handles null→'' for description fields. Removed unused `UserForm`/`LevelForm`/`PageForm` model imports. Build clean, 175/175 tests pass |
| 15 | Add `hasPermission()` method | ✅ DONE | `permissions` signal was `Record<string, boolean>` with no type-safe accessor — any string key accepted, typos silently failed | `hasPermission(key: string): boolean` method already exists in `PermissionService` (line 19-21). Returns `!!this._permissions()[key]`. Used by guards and templates. No further changes needed |
| 16 | Fix `ConfirmDialogService` typing | ✅ DONE | `MatDialog.open()` without generics — `afterClosed()` returned `Observable<any>`, `result` was untyped | `MatDialog.open<ConfirmDialogComponent, ConfirmDialogData, boolean>()` — 3 type parameters (component, data, result). `afterClosed()` now returns `Observable<boolean | undefined>`. Build clean, 175/175 tests pass |
| 17 | Permission matrix debounce/bulk-save | ✅ DONE | Each checkbox toggle immediately fired API call — rapid clicks created N parallel PATCH requests, backend could process out of order | Optimistic UI + debounced bulk-save: toggle queues change in `signal<Map>`, updates UI immediately. `Subject` + `debounceTime(500ms)` auto-flushes. `saveNow()` button for manual flush. `forkJoin` fires all pending requests in parallel. Pending banner shows unsaved count. `isSavingAll` disables checkboxes during save. `takeUntilDestroyed` for subscription. 5 new tests (queue, saveNow, auto-flush, batch, revoke). Build clean, 178/178 tests pass |
| 18 | Fix race condition in refresh dedup | ✅ DONE | Race window between `finalize` clearing `_refreshCall$` and `shareReplay(1)` completing emission — concurrent 401 could start second refresh | Added `_isRefreshing` boolean flag set synchronously BEFORE creating the refresh observable. Cleared in `finalize` alongside `_refreshCall$ = null`. Two-layer protection: `_isRefreshing` (sync guard) + `_refreshCall$` (observable cache). Even if `_refreshCall$` is null during the microsecond between finalize and shareReplay completion, the boolean guard prevents re-entry. Build clean, 175/175 tests pass |
